    ...
    public function readAll($len) {
        ...
        while (true) {
            ...
            if ($buf === false || $buf === '') {
                ...
                if ($md['timed_out']) {
                    throw new TException('TSocket: timed out reading ' . $len . ' bytes from ' . $this->host_ . ':' . $this->port_);
                } else {
                    throw new TException('TSocket: Could not read ' . $len . ' bytes from ' . $this->host_ . ':' . $this->port_);
                }
        ...
        while (strlen($buf) > 0) {
            ...
            if ($got === 0 || $got === false) {
                ...
                if ($md['timed_out']) {
                    ...
                    throw new TException('TSocket: Could not write ' . strlen($buf) . ' bytes ' . $this->host_ . ':' . $this->port_);
                }
            }
            $buf = substr($buf, $got);
        }
